  ...
  (cond [(var? e) 
        ...
        [(fst? e)
         (let ([v1 (eval-under-env (fst-e e) env)])
           (if (apair? v1)
               (apair-e1 v1)
               (error "Not a pair given to fst as expression")))]
        [(snd? e)
         (let ([v1 (eval-under-env (snd-e e) env)])
           (if (apair? v1)
               (apair-e2 v1)
               (error "Not a pair given to snd as expression")))]
        [(aunit? e) e]
        [(isaunit? e)
         ...
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (not (closure? v1))
               ...
               (let* ([fname (fun-nameopt (closure-fun v1))]
                     ...
                     [clenv (if (false? fname)
                                (cons (cons argname v2) (closure-env v1))
                                (cons (cons fname v1)
                                      (cons (cons argname v2)
                                      (closure-env v1))))]
                     [cbody (fun-body (closure-fun v1))])
                 (eval-under-env cbody clenv))))]
...
(define mupl-map
  (fun #f "fn-for-map"
       (fun "f" "mlst"
            (ifaunit (var "mlst")
                (aunit)
                (apair (call (var "fn-for-map") (fst (var "mlst")))
                       (call (var "f") (snd (var "mlst"))))))))
...
(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "int-i"
             (fun #f "mlst"
                  (call
                   (call (var "map") (fun #f "x" (add (var "x") (var "int-i"))))
                   (var "mlst"))))))
